pub mod ui {
    ...
    fn render_octave(option: &str, his_number: u8) -> yew::Html {
        let handle_mousedown =
            |number: u8| yew::Callback::from(move |_: yew::MouseEvent| piano_touch(number, true));
        let handle_mouseup =
            |number: u8| yew::Callback::from(move |_: yew::MouseEvent| piano_touch(number, false));
        let handle_mouseleave =
            |number: u8| yew::Callback::from(move |_: yew::MouseEvent| piano_touch(number, false));

        yew::html! {
            <div>
                <li class={option.to_string()}
                    onmousedown={handle_mousedown(his_number)}
                    onmouseup={handle_mouseup(his_number)}
                    onmouseleave={handle_mouseleave(his_number)}>
                </li>
            </div>
        }
    }

    fn piano_touch(button_number: u8, stop: bool) {
        let connection_out = create_connection_out();
        play_notes(button_number, connection_out, stop);
    }

    fn play_notes(number: u8, mut connection_out: midir::MidiOutputConnection, stop: bool) {
